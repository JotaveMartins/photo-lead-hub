ALTER TABLE public.leads ADD COLUMN IF NOT EXISTS data_entrada_triagem_feita timestamptz;
  ...
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    CASE NEW.status
      ...
      WHEN 'Triagem Feita' THEN
        IF NEW.data_entrada_triagem_feita IS NULL THEN
          NEW.data_entrada_triagem_feita = now();
        END IF;
