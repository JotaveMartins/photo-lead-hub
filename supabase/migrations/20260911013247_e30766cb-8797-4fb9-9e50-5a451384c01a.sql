...
BEGIN
  ...
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    -- Mutually exclusive closing dates
    IF NEW.status <> 'Fechado Ganho' THEN
      NEW.data_entrada_fechado_ganho = NULL;
    END IF;
    IF NEW.status <> 'Fechado Perdido' THEN
      NEW.data_entrada_fechado_perdido = NULL;
    END IF;
  END IF;
